...
mi_x, mi_y, ma_x, ma_y = -12288, -6144, 12288, 6144
...
for lod in range(1, 4):
    ...
    for tx in range(-1, lw + 1):
        for ty in range(-1, lh + 1):
            ...
            for sx in range(0, m):
                for sy in range(0, m):
                    key = f"{mi_x + tx * lod_tile_size + sx * 1024},{mi_y + ty * lod_tile_size + sy * 1024}"
            ...
            resized.save(f"./src/tiles/lod-{lod}/{mi_x + tx * lod_tile_size},{mi_y + ty * lod_tile_size}.png")
